...
import { registerSchema, RegisterInput } from "@/schema/registerSchema";

import { useAuthContext } from "@/context/AuthContext";

export default function RegisterPage() {
  const { registerUser } = useAuthContext();
  const { register, handleSubmit, formState: { errors, isValid, isSubmitting } } = useForm<RegisterInput>({
      ...
      <div className="w-full max-w-[420px]">
        ...
        <div className="bg-white dark:bg-[#0c0c0e] border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm p-8">
          <div className="mb-8">
            ...
            <p className="text-sm text-slate-500">Sign up to start organizing your workflow.</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              />
              {errors.username && <span className="text-red-500 text-[10px] font-medium ml-0.5">{errors.username.message}</span>}
            </div>
            ...
            <button
              ...
              className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2.5 rounded-lg text-sm transition-colors shadow-sm disabled:opacity-50 mt-4"
            >
